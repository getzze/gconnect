...
static gchar *__mconn_get_uuid(MconnCryptPrivate *priv)
{
    ...
    char *subj = X509_NAME_oneline(X509_get_subject_name(priv->cert), NULL, 0);
    char *issuer = X509_NAME_oneline(X509_get_issuer_name(priv->cert), NULL, 0);
    g_assert(strcmp(subj, issuer) == 0);

    /* dup the key as buffer goes away with BIO */
    uuid = g_strndup(issuer, 37);
    return uuid;
}
